...
#include "../../../examples/lighting-app/telink/include/lds_mfg_token_info_config.h"
...
bool LoadDACCertAndKey(uint8_t * buffer, struct FactoryData * factoryData)
{
    ...
    const struct device * flash_token = TOKEN_PARTITION_DEVICE;

    lds_token_info_t token = {0};
    memset(&token, 0xFF, sizeof(token));

    flash_read(flash_token, TOKEN_PARTITION_OFFSET + OFFSET_START_ADDRESS, &token, sizeof(token));

    if (0xFF == token.matter_dac_key)
    {
    ...
    {
        uint8_t default_dac_key[16] = {0xa0, 0xa1, 0xa2, 0xa3, 0xa4, 0xa5, 0xa6, 0xa7, 0xa8, 0xa9, 0xaa, 0xab, 0xac, 0xad, 0xae, 0xaf};
        aes_decrypt(default_dac_key, buffer + 2, dac_key_decrypt);
        aes_decrypt(default_dac_key, buffer + 18, dac_key_decrypt + 16);
        factoryData->dac_priv_key.data = dac_key_decrypt;
    }
